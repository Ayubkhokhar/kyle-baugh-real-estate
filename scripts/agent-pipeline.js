...
import { registerAgentInCodebase, toIdentifier } from "./auto-register.js";
...
export async function runAgentPipeline({
  ...
  logCallback(`Starting autonomous pipeline for '${slug}' (identifier: '${ident}') from ${url}...`);
  ...
  if (h1Text.toLowerCase().includes("find an agent") || titleText.includes("Real Estate Agents, Profiles & Listings - Compass")) {
    throw new Error(`Compass URL redirected to the generic search directory. No agent profile exists at: ${url}`);
  }
  ...
  if (!agentName) {
    agentName = h1Text || titleText.split("|")[0].trim();
  }
  ...
  // Download Headshot
  let localHeadshotPath = `/images/compass/${ident}-headshot.webp`;
  if (rawHeadshotUrl) {
    const headshotFile = path.join(IMAGES_DIR, `${ident}-headshot.webp`);
  ...
  for (let i = 0; i < rawCards.length; i++) {
    ...
    const localImgName = `${ident}-${propSlug}-cover.webp`;
    ...
    if (!isPastSale && detailHref) {
      ...
      for (let pIdx = 0; pIdx < detailPhotos.length; pIdx++) {
        ...
        const gImgName = `${ident}-${propSlug}-photo-${pIdx + 1}.webp`;
  ...
  const propertiesFilePath = path.join(AGENTS_DIR, `${ident}Properties.js`);
  fs.writeFileSync(
    propertiesFilePath,
  ...
  logCallback(`✓ Wrote properties file: ${ident}Properties.js`);

  // 4. WRITE PROFILE FILE
  const profileFilePath = path.join(AGENTS_DIR, `${ident}Profile.js`);
  const profileContent = `// Auto-generated Profile for ${agentName}
export const ${ident}Profile = {
...
`;
  ...
  logCallback(`✓ Wrote profile file: ${ident}Profile.js`);
  ...
  if (buildAndDeploy) {
    ...
    try {
      ...
      const errOut = buildErr.stderr ? buildErr.stderr.toString() : buildErr.message;
      logCallback(`❌ Vite build failed: ${errOut}`);
      throw new Error(`Build failed: ${errOut}`);
    }
    ...
    try {
      ...
      logCallback(`⚠ Git push warning: ${gitErr.message}`);
    }
  }
