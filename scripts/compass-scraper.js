...
// Accept target Compass agent URL from CLI arguments or default to Kyle Baugh
const targetUrlArg = process.argv[2];
const COMPASS_AGENT_URL = targetUrlArg || 'https://www.compass.com/agents/kyle-baugh-dallas/';
...
const AGENT_PROFILE_FILE = path.join(process.cwd(), 'src', 'data', 'agentProfile.js');
...
async function runScraper() {
  console.log(`\n======================================================`);
  console.log(`[Universal Compass Scraper] Target: ${COMPASS_AGENT_URL}`);
  console.log(`======================================================\n`);
  ...
  // Fallbacks from HTML if LD+JSON was incomplete
  if (!agentName) {
    agentName = $('h1').first().text().trim() || $('title').text().split('|')[0].trim() || 'Luxury Real Estate Advisor';
  }
  ...
  if (!rawHeadshotUrl) {
    $('img').each((_, img) => {
      ...
      if ((src.includes('/m/') || src.includes('profile')) && (alt.includes(agentName) || src.includes('f607e49d'))) {
        rawHeadshotUrl = src;
      }
    });
  }

  console.log(`[Agent Profile Detected]`);
  console.log(`  Name:   ${agentName}`);
  console.log(`  Phone:  ${agentPhone || 'Not listed'}`);
  console.log(`  Email:  ${agentEmail || 'Not listed'}`);
  console.log(`  Office: ${agentOffice || 'Dallas, TX'}`);
  console.log(`  Headshot: ${rawHeadshotUrl ? 'Found' : 'Using default'}`);

  // Download agent headshot
  let localHeadshotPath = '/images/compass/agent-headshot.webp';
  if (rawHeadshotUrl) {
    const headshotFile = path.join(IMAGES_DIR, 'agent-headshot.webp');
    const downloaded = await downloadImageWithFallback(rawHeadshotUrl, null, headshotFile);
    if (!downloaded) {
      localHeadshotPath = rawHeadshotUrl;
    }
  }

  // Save agentProfile.js
  const agentProfileContent = `// Auto-generated by scripts/compass-scraper.js
export const agentProfile = {
  advisorName: "${agentName}",
  title: "Lead Real Estate Advisor & Luxury Specialist",
  brokerage: "Compass RE Texas, LLC",
  phone: "${agentPhone || '214.980.3933'}",
  phoneTel: "${(agentPhone || '2149803933').replace(/[^0-9]/g, '')}",
  email: "${agentEmail || 'kyle.baugh@compass.com'}",
  officeAddress: "${agentOffice || '6220 Gaston Avenue, Suite 100, Dallas, TX 75214'}",
  ...
  profileUrl: "${COMPASS_AGENT_URL}"
};
`;
  fs.writeFileSync(AGENT_PROFILE_FILE, agentProfileContent, 'utf8');
  console.log(`[Agent Profile] Saved to ${AGENT_PROFILE_FILE}`);

  // --- 2. EXTRACT PROPERTIES & LISTINGS ---
